class StocksSoldPerPerson < ApplicationRecord
  ...
  def self.find_by_user_id(user_id, date, pattern)
    ...
    StocksSoldPerPerson.joins("INNER JOIN stocks on stock_id = stocks.id").where('created_at > ? AND user_id = ? AND stocks.name like ?', user_id, compare_date, "%#{pattern}%").order(created_at: :desc)
  end
